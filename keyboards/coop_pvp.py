...
def get_coop_battle_keyboard(
    ...
    if force_response:
        if action_state.get("special_variant_key"):
            ...
            rows.append([
                InlineKeyboardButton(
                    ...
                    callback_data=f"coop_action_special_pick_{action_state['special_variant_key']}_base",
                ),
                InlineKeyboardButton(
                    text=f"⚡ Усиленный ({action_state.get('special_variant_amp_cost', 0)} CE)",
                    callback_data=f"coop_action_special_pick_{action_state['special_variant_key']}_amp",
                ),
            ])
            rows.append([InlineKeyboardButton(text="🔙 Назад", callback_data="coop_action_special_variant_back")])
            return InlineKeyboardMarkup(inline_keyboard=rows)
        ...
        if battlerdan_stage:
            ...
            if battlerdan_stage == "topic":
                prefix = "coop_action_battlerdan_topic"
                label = "📚 Выбери тему"
            elif battlerdan_stage == "choose":
                prefix = "coop_action_battlerdan_choose"
                label = "🗣 Выбери правду"
            else:
                prefix = "coop_action_battlerdan_guess"
                label = "❓ Угадай ответ"
    ...
    if action_state.get("can_sword", False):
        ...
        rows.append([InlineKeyboardButton(text=f"⚖️ {sword_name}", callback_data="coop_action_higuruma_sword")])
            ...
            if special.get("variants"):
                rows.append([
                    InlineKeyboardButton(
                        ...
                        callback_data=f"coop_action_special_menu_{special['key']}",
                    )
                ])
                continue
            rows.append([
